	...
	$(document).ready(function(){
		...
		homexhttp.onreadystatechange = function() {
			...
			if (this.readyState == 4 && this.status == 200) {
				...
				//alert(homepage_response.code);
				sectionObj=homepage_response.section_name;
				
				//alert(homepage_response.section_name.length);
				...
				for(i=0;i<homepage_response.section_name.length;i++){
					...
					section_name =homepage_response.section_name[i]['title'];
					section_id =homepage_response.section_name[i]['section_id'];
					
					//alert(i);
					//alert(section_name);
					//alert(section_id);
					//alert(homepage_response.section_name[i].data.length);
		...
		function callgetAppFeatureContent(section_id,section_name,sectionindex) {  
			...
			sanjayObj[section_id]=sectionObj[sectionindex].data;
			
		
			
			total_Section_id.push(section_id);

						for(j=0;j<sectionObj[sectionindex].data.length;j++){
							
							
						
							//console.log(appfeaturecontentresponse.section[j]);
							//console.log('load');
							var section =[sectionObj[sectionindex].data[j]['permalink'],
							              sectionObj[sectionindex].data[j]['content_types_id'],
							              sectionObj[sectionindex].data[j]['name'],
							              sectionObj[sectionindex].data[j]['story'],
							              sectionObj[sectionindex].data[j]['video_duration'],
							              sectionObj[sectionindex].data[j]['genre'],
							              sectionObj[sectionindex].data[j]['banner'],
							              sectionObj[sectionindex].data[j]['release_date'],
							              sectionObj[sectionindex].data[j]['posterForTv'],
							              sectionObj[sectionindex].data[j]['video_duration']
							];
							permalink=[sectionObj[sectionindex].data[j]['permalink']];
							content_types_id=[sectionObj[sectionindex].data[j]['content_types_id']];
							name=[sectionObj[sectionindex].data[j]['name']];
							story=[sectionObj[sectionindex].data[j]['story']];
							genre = [sectionObj[sectionindex].data[j]['genre']];
							banner = [sectionObj[sectionindex].data[j]['banner']];
							release_date=[sectionObj[sectionindex].data[j]['release_date']];
							posterForTv=[sectionObj[sectionindex].data[j]['posterForTv']];
							video_duration=[sectionObj[sectionindex].data[j]['video_duration']];

						
							
						feature_content_detail=feature_content_detail+'<div class="col-3 card border-inside"  data-row="row'+sectionindex+'" data-contenttypes='+sectionObj[sectionindex].data[j]['content_types_id']+' data-perma='+sectionObj[sectionindex].data[j]['permalink']+' id="content_imag_div" data-id='+sectionObj[sectionindex].data[j]['content_types_id']+' data-sectionid='+section_id+' data-index='+j+'>'+
                        '<a href="#" data-sectionid='+section_id+' id="img_'+section_id+'_'+j+'">'+
                        '<div class="hmpageimg">'+
 	                     '<img id='+sectionObj[sectionindex].data[j]['permalink']+' src='+sectionObj[sectionindex].data[j]['posterForTv']+' data-id='+sectionObj[sectionindex].data[j]['content_types_id']+' data-sectionid='+section_id+' data-index='+j+' class="img-fluid" alt="">'+
			             '</div>'+
                        '</a>'+
                        '</div>'
		...
		function callgetcontentdetails(sectionId,sectionIndex){
			...
			if(sanjayObj[sectionId][sectionIndex]['story']!=null){
				var story=truncate(sanjayObj[sectionId][sectionIndex]['story']);
				var story=story;
			}else{
				var story="";
			}			
			
			if(sanjayObj[sectionId][sectionIndex].name!=null){
				var name=sanjayObj[sectionId][sectionIndex].name;
			}else{
				var name='';
			}
			if(sanjayObj[sectionId][sectionIndex].release_date!=null){
				var year=sanjayObj[sectionId][sectionIndex].release_date;
				var release_date = year.split("-", 1);
			}
			else{
				var release_date='';
			}
			if(sanjayObj[sectionId][sectionIndex].genre!=null){
				var genre=sanjayObj[sectionId][sectionIndex].genre;
			}
			else{
				var genre='';
			}
...
//			alert(convertToText(duration));
			...
			if (sanjayObj[sectionId][sectionIndex].banner!=null) {
				
				var body = document.getElementsByClassName('banner-home')[0];
				
				body.style.backgroundImage = 'url(' + sanjayObj[sectionId][sectionIndex].banner + ')';
...
/*		document.addEventListener('keydown', function(e) {
        ...
        if (e.keyCode == '38') {
           ...
           //console.log("left");
			...
			var player_href = "Jwplayer.html?conunter="+sectionIndex;
