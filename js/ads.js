...
function init() {
  ...
  count = getParameterByName("conunter",window.location.href);
...
function onAdfinished(adEvent) {
	...
	if(adEvent.type == google.ima.AdEvent.Type.ALL_ADS_COMPLETED){
		...
		if(localStorage.getItem("response")==null || localStorage.getItem("response")==undefined || localStorage.getItem("response")==""){
			...
			var category_response = JSON.parse(retrievedObject);		
			category_list=category_response.section_name;
		...
		for(i=0;i<category_list.length;i++){
			...
			section_id =category_list[i]['section_id'];
			
			sanjayObj[section_id]=category_list[i].data;
...
//			
//						  
//			var url=category_list[count].sources[0].file;
//		
...
function onAdError(adErrorEvent) {
  ...
  if(localStorage.getItem("response")==null || localStorage.getItem("response")==undefined || localStorage.getItem("response")==""){
		...
		var retrievedObject = localStorage.getItem('response');
		var category_response = JSON.parse(retrievedObject);		
		category_list=category_response.playlist;
